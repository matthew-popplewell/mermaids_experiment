...
#
# Usage:
#   ./scripts/start_server.sh          # Auto-detect and start all connected mounts
#   ./scripts/start_server.sh 1        # Start with 1 mount instance
#   ./scripts/start_server.sh 4        # Start with 4 mount instances
#
...
if [ "$NUM_MOUNTS" -eq 1 ]; then
    ...
    # Use mount-multi CLI to auto-assign ports and connect
    if command -v mount-multi &> /dev/null; then
        mount-multi connect
    else
        # Fallback to direct Python call
        python -c "from mount_driver.multi_mount import MultiMountController; MultiMountController().auto_connect()"
    fi
fi
...
if [ "$NUM_MOUNTS" -eq 1 ]; then
    echo "Use 'mount-single' for single mount control"
else
    echo "Mounts appear as: Mount 1, Mount 2, etc."
    echo "Use 'mount-multi status' to check connections"
fi
