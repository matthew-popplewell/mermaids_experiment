...
"""
GPS Serial Module - Direct connection to Adafruit Ultimate GPS

Reads GPS coordinates directly from a USB GPS receiver via serial port.
Based on the approach used in asi_driver, adapted for mount control.

No gpsd daemon required - connects directly to the GPS hardware.

Requirements:
    Python: pyserial, pynmea2

Usage:
    from gps_serial import get_gps_location, gps_available, format_location
...
def gps_available(port: Optional[str] = None) -> bool:
    ...
    Returns:
        bool: True if GPS device can be found
    """
    if port:
        try:
...
class GPSReader:
    """Direct serial reader for Adafruit Ultimate GPS module."""
...
def get_gps_location(
    ...
    gpsd-based gps_location module for easy drop-in replacement.
    ...
    return {
        ...
        'accuracy': location.hdop * 5 if location.hdop else None,  # Rough estimate
    }
...
def format_location(location: dict) -> str:
    ...
    Returns:
        str: Formatted multi-line string for display
    """
    lat = location['lat']
    lon = location['lon']
...
if __name__ == '__main__':
    ...
    if not port:
        ...
        print("Connect a USB GPS receiver and try again.")
        sys.exit(1)

    print(f"\nDetected GPS on: {port}")
    ...
    try:
        location = get_gps_location(timeout=60, progress_callback=progress)
        print("\n")
        print(format_location(location))
